...
/// @notice Send ‘_amount‘ tokens to ‘_to‘ from ‘_from‘ on the condition 
...
/// on its behalf
/// @param _spender The address of the account able to transfer token 
...
function transfer(address _to, uint256 _amount) noEther 
returns (bool success) {
...
/// @notice Create Token with ‘_tokenHolder‘ as initial owner of the Token
...
/// @return The divisor used to calculate the token creation rate during    /// the creation phase
...
/// @param _minTokensToCreate Minimum required wei-equivalent tokens
/// to be created for a successful DAO Token Creation
/// @param _closingTime Date (in Unix time) of the end of DAO Token Creation
/// @param _privateCreation If zero DAO Token Creation open to public, a
/// non-zero address means that the DAO Token Creation is only for the 
/// address
...
proposals.length = 1; // avoids prop with ID 0 because it is used
...
if (_debatingPeriod > 8 weeks)
	...
	function executeProposal(
		...
		bytes _transactionData
			...
			? splitExecutionPeriod : executeProposalPeriod;
...
// Check if the proposal can be executed
		...
		if (now < p.votingDeadline // has the voting deadline arrived? || !	p.open
